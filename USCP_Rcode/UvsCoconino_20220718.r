...
#Quick function to pull in my kml files of interest  
...
#want to look at which points are in the coconino

#finding which points are the in various polygons
...
p.f01 <- ggplot() + 
  ...
  geom_point(aes(x = fin01[,1], y = fin01[,2]), 
             color = 'black', alpha = 0.5,
             pch = 16)
p.f01 # nice filter  - so method works. nice to see a visual check

#Pulling average U for 0-1
#data is not exactly log normal but it's not normal either 
#closer to lognormal
flu01 <- log10(fin01[,3])
...
p.f13 <- ggplot() + 
  ...
  geom_point(aes(x = fin13[,1], y = fin13[,2]), 
             color = 'black', alpha = 0.5,
             pch = 16)
p.f13 # nice visual check  - looks good
...
p.f325 <- ggplot() + 
  ...
  geom_point(aes(x = fin325[,1], y = fin325[,2]), 
             color = 'black', alpha = 0.5,
             pch = 16)
p.f325 # nice visual check  - looks good
...
boxplot(as.numeric(as.character(T.U[,1]))~as.character(T.U[,2]),
        data=T.U) 
#will make fancy later #all values are lower but plot looks the same

#ANOVA
AOO.aov <- aov(as.numeric(as.character(T.U[,1]))
               ~ as.character(T.U[,2]), data = T.U)
summary(AOO.aov) #significant 7*10^-6
TukeyHSD(AOO.aov) #3 to 25 is higher than other two # ok

#Kruskal Wallis #dunnett test requires DescTools
require(DescTools)
CCs <- kruskal.test(as.numeric(as.character(T.U[,1]))
                    ~ as.character(T.U[,2]), data = T.U) 
CCs # p = 0.0003
DunnettTest(as.numeric(as.character(T.U[,1]))
            ~ as.factor(T.U[,2]), data = T.U) 
#highest salinity is higher; p = 4*10^-5 

### Surface Analysis of Coconino and U 

uinCsr <- as.numeric(is.na(sp::over(uspat,CCAq)[,1]) == FALSE) #1 in Aq, 0 nn Aq
uinCsr <- as.data.frame(uinCsr)

finCsr <- Uc[which(uinCsr[,1] == 1),] # in Surface Aq
fnnCsr <- Uc[which(uinCsr[,1] == 0),] # in Surface Aq 
