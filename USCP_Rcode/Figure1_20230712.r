...
AZ_CP <- readJPEG('F1_PanelAwMrksN_HD.JPG', native = TRUE) 
...
#Pulling in primary inset
USi <- readJPEG('SmallUSContigBox.JPG', native = TRUE) 
...
p.Pa <- ggplot() + 
  ...
  geom_segment(
    ...
    color = "light gray") + # EW Boundary CO - UT
  geom_segment(
  ...
  annotate(geom="text", x=-112, y=39, label="Utah",
           color="white", size=6)+
  annotate(geom="text", x=-107.5, y=37.5, label="Colorado",
           color="white", size=6)+
  ...
  coord_cartesian(ylim = c(32.998294, 41.152667), xlim =c(-114.149388, -106.356675), clip = "on")+
  annotation_custom(rasterGrob(USi,
                               width = unit(1,"npc"),
                               height = unit(1,"npc")),
                    -109.03, -106.4, 39.76, 41.12)+
  #coord_fixed(1.3) +
...
IS_CP <- readJPEG('F1_PanelBwoMrk_HD.JPG', native = TRUE) 
...
#Bring in x-section
XS <- qimpt("U_XsectionPath.KML")

XSP <-as.data.frame(XS$U_XsectionPath.kmz@lines[1][[1]]@Lines[[1]]@coords)
...
p.Pb <- ggplot() +
  annotation_custom(rasterGrob(IS_CP,
                               ...
                               height = unit(1,"npc")),
                    -112, -109, 34.8, 37.5)+
  annotate(geom="text", x=-111.9, y=37.35, label="B",
           color="white", size=8)+
  geom_polygon(data = iset, aes(x=long, y = lat, group = group), 
  ...
  geom_path(data=XSP, aes(x=V1, y = V2), color = "white", size = 1.5)+
  geom_segment(
  ...
  annotate(geom="text", x=-111.20, y=35.1, label="50 km",
           color="white", size=6)+
  annotate(geom="text", x=-109.1, y=36.4, label="X",
           color="white", size=6)+
  annotate(geom="text", x=-110.8, y=34.91, label="X'",
           color="white", size=6)+
  annotate(geom="text", x=-111.45, y=36.15, label="Y",
           color="white", size=6)+
  geom_segment(
