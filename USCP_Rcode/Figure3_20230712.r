...
p.3 <- ggplot() +  
  ...
  geom_point(aes(x = -109.5525, y = 36.1544),
             color = 'dark gray', fill= "white", pch = 23, cex = 3)+
  annotate("text", x=-109.3, y=36.1, label="Chinle",
           color ="white", parse=TRUE, size = 5)+
  ...
  annotate("text", x=-111.44, y=34.7, label="Uranium*' Concentration'", 
           parse=TRUE)+
  ...
  annotate(geom="text", x=-111.20, y=35.1, label="50 km",
           color="white", size=5)+
  geom_segment(
    aes(x = -111.7, y = 35.4, xend = -111.7, yend = 35.6), 
    color = "white", size = 1, 
    arrow = arrow(length = unit(0.3, "cm"), type = "open", ends = "last"))+
  annotate(geom="text", x=-111.7, y=35.3, label="N",
           color="white", size=6)+
...
plot(p.3) # Plotting  exports as 5 x 5
